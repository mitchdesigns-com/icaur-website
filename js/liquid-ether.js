/* ═══════════════════════════════════════════════════════════
   LiquidEther — vanilla JS port of the React Bits component
   Requires: three (via importmap)
   ═══════════════════════════════════════════════════════════ */
...
export function createLiquidEther(container, opts = {}) {
  ...
  let webgl = null;
  let isVisible = true;
  let ro = null, io = null, resizeRaf = null;
  ...
  class Simulation {
    constructor(o) {
      this.options={ iterations_poisson:32, iterations_viscous:32, mouse_force:20, resolution:0.5, cursor_size:100, viscous:30, isBounce:false, dt:0.014, isViscous:false, BFECC:true, ...(o||{}) };
  ...
  class WebGLManager {
    constructor(props) {
      this.props=props; this._raf=null;
      ...
      this._onVisibility=()=>{ const h=document.hidden; if(h){this.pause();}else if(isVisible){this.start();} };
    ...
    loop() { if(!this.running)return; this.render(); this._raf=requestAnimationFrame(this._loop); }
  ...
  io = new IntersectionObserver(entries => {
    const e = entries[0], iv = e.isIntersecting && e.intersectionRatio > 0;
    isVisible = iv;
    if (!webgl) return;
